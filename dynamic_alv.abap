REPORT zth_test7.
"In this report, I have a ztable which has already some records, "however, I want to showcase the data and also allow the user "to modify the records if they wish to do so.
"This requirement is very frequent in my case, sometimes, "users want an alv report and also need a functionality
"to change the some of the data and save into the database. "This requirement is so frequent that I have created this "report which will take any z or y table and display its data "in editable mode and users can modify the data of any field "except keyfields.
...
get_data	CHANGING REFERENCE(ch_ref) TYPE REF TO data RAISING lcl_exception, get_components CHANGING REFERENCE(ch_components) TYPE cl_abap_structdescr=>component_table, create_table IMPORTING REFERENCE(im_components) TYPE cl_abap_structdescr=>component_table
...
IMPORTING
r_salv_table	= DATA(lo_tab)	" Basis Class Simple ALV Tables CHANGING
t_table	= <ft_table2>
).
...
r_columns	= lo_tab->get_columns( )	" ALV Filter r_aggregations = lo_tab->get_aggregations( )	" ALV Aggregations RECEIVING
...
DATA : lr_table TYPE REF TO cl_abap_tabledescr. DATA : lr_data TYPE REF TO data.
me->get_components( CHANGING ch_components = lt_components ).

IF lines( lt_components ) IS NOT INITIAL. lr_table = cl_abap_tabledescr=>create(
p_line_type = cl_abap_structdescr=>create( p_components = lt_components ) p_table_kind = cl_abap_tabledescr=>tablekind_std
). ELSE.
RAISE EXCEPTION TYPE lcl_exception EXPORTING
...
"Design Field Catalogue me->create_fcat( EXPORTING
im_ref_tab = lo_ref_sel CHANGING
ch_fcat	= fcat
).
...
DATA : gt_components TYPE cl_abap_structdescr=>component_table. ASSIGN lo_ref->* TO FIELD-SYMBOL(<ft_table>).
...
METHOD get_data.
FIELD-SYMBOLS : <ft_table> TYPE table. ASSIGN ch_ref->* TO <ft_table>.
...
"Get the table data into the report me->get_data(
CHANGING
ch_ref = lo_ref
).

"Create a new table with one column named SELECT me->create_table(
EXPORTING
...
*&	*
MODULE status_0100 OUTPUT. SET PF-STATUS 'TEST'.
...
"Check if there is any data changed. IF lo_obj->lo_grid IS BOUND.
...
"Modify the same Z or Y table from the selected rows. MODIFY (lcl_alv=>tabname) FROM TABLE <fs_table>.
ENDFORM.
*&	*
*&	Form REFRESH
*&	*
FORM refresh .
DATA : stable TYPE lvc_s_stbl. DATA : m_itab TYPE bapiret2_tab.

CALL METHOD lo_obj->lo_grid->set_ready_for_input EXPORTING
i_ready_for_input = 0.
...
CALL METHOD lo_obj->lo_grid->refresh_table_display EXPORTING
is_stable = stable EXCEPTIONS
finished = 1
OTHERS	= 2.
